...
class ChannelsManagement:
    ...
    async def list_channels(self, event):
        ...
        for i, channel in enumerate(channels[:10], 1):  # Show max 10 channels
            ...
            buttons.append([
                Button.inline(f"🗑️ حذف {i}", f"delete_channel_{channel_id}"),
                Button.inline(f"✏️ تعديل {i}", f"edit_channel_{channel_id}")
            ])
    ...
    async def edit_channel(self, event, channel_id):
        ...
        try:
            ...
            buttons = [
                [Button.inline("🔄 تحديث المعلومات", f"refresh_channel_{channel_id}")],
    ...
    async def finish_add_channels(self, event):
        ...
        try:
            ...
            if not state_data or state_data.get('state') != 'waiting_multiple_channels':
                await event.answer("❌ لا توجد عملية إضافة قنوات نشطة")
                return

            channels = state_data.get('data', {}).get('channels', [])
